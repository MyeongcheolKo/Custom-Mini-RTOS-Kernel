...
int main(void)
{
	...
	while(1)
	{
		...
		command_code = 0x67;
		I2C_Master_send(&I2C1_Handle, &command_code, 1, SLAVE_ADDR, I2C_SR_ENABLE);
		I2C_Master_receive(&I2C1_Handle, &len, 1, SLAVE_ADDR, I2C_SR_ENABLE);
		printf("Receiving message of length: %d\n", len);

		//get the message from slave
		command_code = 0x76;
		I2C_Master_send(&I2C1_Handle, &command_code, 1, SLAVE_ADDR, I2C_SR_ENABLE);
		I2C_Master_receive(&I2C1_Handle, received_mssg, len, SLAVE_ADDR, I2C_SR_DISABLE);
